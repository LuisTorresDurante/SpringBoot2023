...
public class RentalPDFExporter {
    ...
    public void export(HttpServletResponse response) throws DocumentException, IOException {
        ...
        Paragraph pReturnDateLabel = new Paragraph("ÚLTIMA ACTUALIZACIÓN:", fontBodyBold);
        pReturnDateLabel.setAlignment(Paragraph.ALIGN_CENTER);
        document.add(pReturnDateLabel);

        Paragraph pReturnDate = new Paragraph(dateFormat.format(new Date(rental.getLastUpdate().getTime())), fontBody);
        pReturnDate.setAlignment(Paragraph.ALIGN_CENTER);
        document.add(pReturnDate);
        
/*        Image barcode = Image.getInstance("src/main/resources/static/assets/imgs/barcode.png");
        barcode.scaleAbsolute(156, 41);
        barcode.setAbsolutePosition(30, document.bottom());
